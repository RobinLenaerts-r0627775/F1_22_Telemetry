...
public class ParticipantData
{
    ...
    public static ParticipantData FromArray(byte[] packet)
    {
        ...
        s.m_aiControlled = reader.ReadByte();
        s.m_driverId = reader.ReadByte();
        s.m_networkId = reader.ReadByte();
        s.m_teamId = reader.ReadByte();
        s.m_myTeam = reader.ReadByte();
        s.m_raceNumber = reader.ReadByte();
        s.m_nationality = reader.ReadByte();
        for (int i = 0; i < 48; i++)
        {
